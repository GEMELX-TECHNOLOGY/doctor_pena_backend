...
async function isChildPatient(patient_id) {
	const [rows] = await query("SELECT patient_type FROM Patients WHERE id = ?", [
		patient_id,
	]);
...
exports.registerVaccine = async (req, res) => {
	try {
		const { patient_id, name, application_date, dose, next_dose, notes } =
			req.body;
		...
		if (!(await isChildPatient(patient_id))) {
			return res.status(400).json({
				error:
					"Las vacunas solo pueden registrarse para pacientes bebés o niños",
			});
		}
...
exports.getVaccinesByPatient = async (req, res) => {
	try {
		...
		const [rows] = await query("SELECT * FROM Vaccines WHERE patient_id = ?", [
			patientId,
		]);
		res.json({
			success: true,
...
exports.deleteVaccine = async (req, res) => {
	try {
		const vaccineId = req.params.id;
		const [result] = await query("DELETE FROM Vaccines WHERE id = ?", [
			vaccineId,
		]);
