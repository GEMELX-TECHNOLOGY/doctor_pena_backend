...
async function registerUser(req, res, allowedRoles) {
    try {
        ...
        if (role === 'medico') {
            ...
            if (!first_name || !last_name || !specialty || !license_number || !schedule || !office) {
                return res.status(400).json({
            ...
            await query(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)`,
                [insertId, first_name, last_name, specialty, license_number, schedule, office]
            );
        }
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    if (!token || !newPassword) {
      return res.status(400).json({ error: 'Se requiere el token y la nueva contraseña' });
    }
...
exports.updateAdminCredentials = async (req, res) => {
  try {
    ...
    if (req.user?.role !== 'admin') {
      return res.status(403).json({ error: 'Solo los administradores pueden actualizar sus credenciales' });
    }
    ...
    res.status(500).json({ error: 'Error interno al actualizar las credenciales' });
  }
};
...
exports.updatePatientCredentials = async (req, res) => {
  try {
    ...
    if (req.user?.role !== 'paciente') {
      return res.status(403).json({ error: 'Solo los pacientes pueden actualizar sus credenciales' });
    }
...
exports.deactivatePatientAccount = async (req, res) => {
  try {
    ...
    if (role !== 'paciente') {
      return res.status(403).json({ error: 'Solo los pacientes pueden desactivar su cuenta' });
    }

    const [user] = await query('SELECT * FROM Users WHERE id = ? AND role = "paciente"', [patientId]);

    if (!user) {
      return res.status(404).json({ error: 'Paciente no encontrado' });
    }

    await query('UPDATE Users SET status = "inactivo" WHERE id = ?', [patientId]);
