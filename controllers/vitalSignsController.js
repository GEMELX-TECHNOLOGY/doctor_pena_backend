...
async function consultationExists(consultation_id) {
	const [res] = await query("SELECT id FROM Consultations WHERE id = ?", [
		consultation_id,
	]);
	return res.length > 0;
}
...
exports.createVitalSigns = async (req, res) => {
	try {
		...
		const [exists] = await query(
			"SELECT id FROM VitalSigns WHERE consultation_id = ?",
			[consultation_id],
		);
		if (exists.length > 0) {
			return res.status(409).json({
				error: "Ya existen signos vitales registrados para esta consulta",
			});
		}
