...
exports.uploadDocument = async (req, res) => {
  try {
    const { registration_number, title, description, type, notes, status } =
      req.body;
...
exports.getAllPendingAnalisys = async (req, res) => {
  try {
    const [docs] = await query(`
		SELECT D.*, Pa.first_name, Pa.last_name
		FROM Documents D 
		JOIN Patients Pa on D.patient_id = Pa.id 
		WHERE D.status = 'pendiente' AND D.type = 'estudio'
		ORDER BY D.date DESC
			`);
    res.json({
      success: true,
...
exports.getAllReadDocuments = async (_req, res) => {
  try {
    const [docs] = await query(`
      SELECT D.title, D.description, D.notes, D.status, Pa.first_name, Pa.last_name 
      FROM Documents D
      JOIN Patients Pa ON D.patient_id = Pa.id
